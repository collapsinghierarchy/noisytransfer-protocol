...
async function readJSON(res) {
  ...
  catch (e) { throw new Error(`Invalid JSON (${res.status}): ${text}`); }
}
...
test('HTTP objects API: end-to-end (create → upload → manifest → commit → ranges)', { timeout: 60_000 }, async (t) => {
...
// after commit:
{
  ...
  assert.equal(resHead.headers.get('accept-ranges'), 'bytes', 'HEAD must advertise Accept-Ranges: bytes');
  ...
  assert.ok(resR1.headers.get('content-range')?.startsWith('bytes 0-'), 'Content-Range missing/invalid for first range');

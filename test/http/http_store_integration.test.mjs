...
test('http_store: create → upload → manifest → 409 → commit → ranges → full → 404', { timeout: 60_000 }, async () => {
  ...
  const manifest = {
    version: 1,
    aead: 'AES-GCM', tagBytes: 16, chunkBytes: CHUNK,
    totalBytes, totalChunks, lastChunkPlaintextBytes: lastPt, counterStart: 0,
    encTag: 'demo', cipherDigest: '', finSigAlg: 'RSA-PSS-SHA256', finSignature: '',
  ...
  // Range checks (parallel)
  {
    ...
    await Promise.all(ranges.map(async ([start, end]) => {
      ...
      const gotStart = Number(m[1]), gotEnd = Number(m[2]), total = Number(m[3]);
  ...
  // Unknown object → 404
  let notFound = false;
  try { await store.get({ objectId: 'not-a-real-id' }); }
  catch (e) { assert.ok(e instanceof NoisyError); assert.equal(e.code, 'NC_NOT_FOUND'); notFound = true; }

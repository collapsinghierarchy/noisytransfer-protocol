...
/** rooms: Map<room, { A?:WebSocket, B?:WebSocket }> */
const rooms = new Map();
...
wss.on("connection", (ws, request) => {
  ...
  if (!room || !/^[A-Za-z0-9-_.]{1,100}$/.test(room) || !/[AB]/.test(side)) {
  ...
  const cleanup = () => {
    ...
    if (e) { delete e[side]; if (!e.A && !e.B) rooms.delete(room); }
  };

  ws.on("message", (buf) => {
    const peer = entry[other];
    if (!peer || peer.readyState !== peer.OPEN) return;
    try { peer.send(buf); } catch {}
  });
  ws.on("close", cleanup);
  ws.on("error", cleanup);
});

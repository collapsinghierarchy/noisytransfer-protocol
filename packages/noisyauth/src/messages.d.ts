...
export function makeOffer({ session, msgS, nonceS }: { session: any; msgS: any; nonceS: any }): {
  type: string;
  sessionId: any;
  offer: {
    msgS: string;
...
export function makeReveal({ session, msgR, nonceR }: { session: any; msgR: any; nonceR: any }): {
  type: string;
  sessionId: any;
  reveal: {
    msgR: string;
...
/** optional ack */
export function makeRcvConfirm({ session }: { session: any }): {
  type: string;
  sessionId: any;
};

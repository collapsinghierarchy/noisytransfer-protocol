...
test.skip("courier: receiver first, sender later (async start)", { timeout: 60_000 }, async () => {
...
test.skip("courier: sender first, receiver later (async start)", { timeout: 60_000 }, async () => {
  const room = crypto.randomUUID();
  const sessionId = crypto.randomUUID();
...
test.skip(
  "courier: chaos flaps (receiver inbound mutes/unmutes)",
  { timeout: 90_000 },
  async () => {
    const room = crypto.randomUUID();
    const sessionId = crypto.randomUUID();

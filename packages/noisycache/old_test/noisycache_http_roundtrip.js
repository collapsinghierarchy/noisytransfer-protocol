...
test.skip("aead.js compat: upload→commit→download roundtrip", { timeout: 90_000 }, async () => {
  ...
  const encryptor = await makeAeadChunkEncryptorCompat({
    keyBytes,
    baseIV,
    aadId,
    tagBytes: 16,
  ...
  const decryptor = await makeAeadChunkDecryptorCompat({
    keyBytes,
    baseIV,
    aadId,
    tagBytes: 16,

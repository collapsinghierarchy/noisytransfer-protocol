...
/* -------------------------------------------------------------------------- */
test.skip(
  "HTTP upload + mailbox courier + parallel download — receiver first, chaos flaps",
  { timeout: 120_000 },
  async () => {
    ...
    const enc = await makeAeadChunkEncryptorCompat({
      keyBytes,
      baseIV,
      aadId,
      tagBytes: 16,
...
/* -------------------------------------------------------------------------- */
test.skip(
  "HTTP upload + mailbox courier + parallel download — sender first, double chaos flaps",
  { timeout: 120_000 },
  async () => {
    const store = new HttpStore(BASE_HTTP);
...
test.skip(
  "HTTP upload + courier — long blob, 768KiB chunks, high parallel, chaos flaps (receiver first)",
  { timeout: 180_000 },
  async () => {
    const store = new HttpStore(BASE_HTTP);

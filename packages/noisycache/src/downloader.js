...
export async function downloadAndDecrypt({
  ...
  if (!decryptor || typeof decryptor.openChunk !== 'function') throw new NoisyError({ code: 'NC_BAD_PARAM', message: 'missing decryptor.openChunk' });
  if (!manifest) throw new NoisyError({ code: 'NC_BAD_PARAM', message: 'manifest required (fetch it before calling)' });

  validateManifest(manifest);
  if (manifest.aead !== decryptor.aead || manifest.tagBytes !== decryptor.tagBytes) {
    throw new NoisyError({ code: 'NC_AEAD_MISMATCH', message: 'decryptor does not match manifest AEAD/tagBytes' });
  }
  ...
  async function drain() {
    ...
    try {
      for (;;) {
        ...
        if (onProgress) onProgress({ seq: nextSeqToDrain, ptBytes: slot.pt.length, ctBytes: slot.ct.length, writtenBytes: written });
  ...
  const doneP = new Promise((res, rej) => { resolveOnce = res; rejectOnce = rej; });
  ...
  async function runOne(seq) {
    try {
      ...
      try {
        ...
        throw new NoisyError({ code: 'NC_DECRYPT_FAILED', message: 'decrypt chunk failed', context: { seq }, cause: e });
      }
      if (!(pt instanceof Uint8Array) || pt.length !== ptLen) {
        throw new NoisyError({ code: 'NC_DECRYPT_SIZE', message: 'plaintext length mismatch', context: { seq, expect: ptLen, got: pt?.length } });
      }
  ...
  if (nextSeqToDrain !== totalChunks || slots.size !== 0) {
    throw new NoisyError({ code: 'NC_INTERNAL_ORDER', message: 'not all chunks drained in order', context: { nextSeqToDrain, totalChunks, pending: slots.size } });
  }

  // Verify cipher digest
  const digestHex = sha.digest('hex');
  if (expectCipherDigest && expectCipherDigest !== digestHex) {
    throw new NoisyError({ code: 'NC_HASH_MISMATCH', message: 'ciphertext digest mismatch', context: { expect: expectCipherDigest, got: digestHex } });
  }
  if (manifest.cipherDigest && manifest.cipherDigest !== digestHex) {
    throw new NoisyError({ code: 'NC_HASH_MISMATCH', message: 'manifest cipherDigest mismatch', context: { manifest: manifest.cipherDigest, got: digestHex } });
  }
  ...
  if (verifyKey && manifest.finSignature) {
    try {
      const preimage = Buffer.concat([
        Buffer.from(JSON.stringify({ aead: manifest.aead, tagBytes: manifest.tagBytes, chunkBytes: manifest.chunkBytes, counterStart: manifest.counterStart })),
      ...
      if (!ok) throw new NoisyError({ code: 'NC_SIGNATURE_INVALID', message: 'verify returned false' });
    } catch (e) {
      throw new NoisyError({ code: 'NC_SIGNATURE_INVALID', message: 'final signature invalid', cause: e });
    }
  }

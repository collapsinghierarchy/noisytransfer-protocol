...
export function validateManifest(m) {
  ...
  for (const k of req)
    if (!(k in m)) throw new NoisyError({ code: "NC_BAD_MANIFEST", message: `Missing ${k}` });
  if (m.version !== 1)
    throw new NoisyError({ code: "NC_BAD_MANIFEST", message: `Unsupported version ${m.version}` });
  if (m.aead !== "AES-GCM")
    throw new NoisyError({ code: "NC_BAD_MANIFEST", message: `Unsupported AEAD ${m.aead}` });
  ...
  if (m.totalChunks === 0 && m.totalBytes !== 0)
    throw new NoisyError({ code: "NC_BAD_MANIFEST", message: "totalChunks=0 but totalBytes>0" });
  if (
    m.totalChunks > 0 &&
    (m.lastChunkPlaintextBytes <= 0 || m.lastChunkPlaintextBytes > m.chunkBytes)
  ) {
    throw new NoisyError({
      code: "NC_BAD_MANIFEST",
      message: "lastChunkPlaintextBytes out of range",
    });
  }
}
...
export function ctTotalBytes(m) {
  ...
  return (
    (m.totalChunks - 1) * (m.chunkBytes + m.tagBytes) + (m.lastChunkPlaintextBytes + m.tagBytes)
  );
}
...
export function makeManifest({
  ...
  if (!Number.isInteger(chunkBytes) || chunkBytes <= 0)
    throw new NoisyError({ code: "NC_BAD_PARAM", message: "chunkBytes must be positive int" });
  if (!Number.isInteger(totalBytes) || totalBytes < 0)
    throw new NoisyError({ code: "NC_BAD_PARAM", message: "totalBytes must be >= 0" });
  const totalChunks = totalBytes === 0 ? 0 : Math.ceil(totalBytes / chunkBytes);
  const lastChunkPlaintextBytes =
    totalBytes === 0 ? 0 : totalBytes - (totalChunks - 1) * chunkBytes;
  const m = {
    version: 1,
    aead,
    tagBytes,
    ...
    finSigAlg,
    finSignature,
    context: { aead, ...context, chunkBytes, counterStart },
  };
  validateManifest(m);
  return m;
}

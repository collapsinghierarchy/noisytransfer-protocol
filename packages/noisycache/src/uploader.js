...
function toAsyncIter(source, chunkBytes) {
  ...
  if (source instanceof Uint8Array) {
    return (async function* () {
      ...
      while (off < n) {
        const end = Math.min(off + chunkBytes, n);
        yield source.subarray(off, end);
        off = end;
      }
    })();
  }
...
export async function uploadCiphertext({
  ...
  if (!encryptor || typeof encryptor.sealChunk !== "function")
    throw new NoisyError({ code: "NC_BAD_PARAM", message: "missing encryptor.sealChunk" });
    ...
    for await (const pt of ptIter) {
      ...
      try {
        ...
        throw new NoisyError({
          code: "NC_ENCRYPT_FAILED",
          message: "encrypt chunk failed",
          context: { seq },
          cause: e,
        });
      }
      if (!(ct instanceof Uint8Array))
        throw new NoisyError({
          code: "NC_ENCRYPT_FAILED",
          message: "encryptor returned non-bytes",
          context: { seq },
        });
      sha.update(ct);
      totalPt += ptU8.length;
      totalCt += ct.length;
      if (onProgress)
        onProgress({ seq, ptBytes: ptU8.length, ctBytes: ct.length, sentBytes: totalCt });
      yield ct;
      seq++;
    }
  }
  ...
  if (signingKey) {
    try {
      ...
      if (typeof signingKey.sign === "function") {
        ...
        throw new NoisyError({
          code: "NC_BAD_PARAM",
          message: "No WebCrypto; provide signingKey.sign()",
        });
      }

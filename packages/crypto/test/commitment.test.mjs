...
import {
  ...
  verifyCommitment,
  packCommitment,
  parseCommitment,         // make sure this is “parseCommitment”
  randomNonce
...
test('commitment roundtrip (SHA3-256)', async () => {
  ...
  const { commitment, nonce, alg, label } =
    await computeCommitment(data, { hash: 'SHA3-256', nonceBytes: 24 });
  ...
  assert.equal(
    await verifyCommitment({
      data,
      nonce: parsed.nonce,                 // ensure this says “parsed.nonce”
      commitment: parsed.commitment,       // and this “parsed.commitment”
      hash: parsed.alg,
      label: parsed.label
    }),
    true
